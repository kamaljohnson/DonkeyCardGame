...
def main():
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], "sl:d:i:f:t:h",
                                   ["from=", "to=", "help", "delay=", "ip=", "stdin", "load-file="])  # output=
...
def doListScan(inputList):
    ListOfHosts = list()
    listOfIP = []       #this list will contain all the ips connected to this computer
    ...
    for Host in ListOfHosts:
        ...
        if Host.Status == -1:
            ...
            listOfIP.append(Host.Adress)
        return (listOfIP)
        #time.sleep(ping_delay)             this time delay not required
...
def doRangeScan():
    ...
    to = int(y)
    ListOfHosts = list()
    listOfIP = []           #this list will contain all the ip address connected to this computer
